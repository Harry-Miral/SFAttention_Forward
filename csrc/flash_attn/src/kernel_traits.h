...
struct Flash_kernel_traits {
    ...
    static constexpr int kHeadDimRounded = (kHeadDim_ <= 192)
        ? (((kHeadDim_ + 32 - 1) / 32) * 32)
        : 256;
...
struct Flash_bwd_kernel_traits : public Base {
    ...
    // 在总大小计算中使用 std::max 来确保为复用区域分配足够空间
    ...
    static constexpr int kSmemSize1colblock = kSmemQdOSize
        + (!Is_V_in_regs
        ? kSmemKVSize + kSmemdSSize + std::max(kSmemPSize, kSmemdQSize) // 在这里也使用 max
        : std::max(kSmemKVSize, kSmemKVSize / 2 + kSmemdSSize + std::max(kSmemPSize, kSmemdQSize)));
    // ======================== END: 关键修复 ========================
